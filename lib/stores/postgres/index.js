...
datastore.saveChallenge = function (user, expectedAnswerDigest, callback) {
  pg.connect(conString, function (err, client) {
    if (err) {
      console.log(err);
      callback('Database error');
    ...
    var query = {
      text: 'insert into challenge (account_id, base_keying_id, expected_answer_digest) \
        values ($1, $2, $3) returning challenge_id',
      values: [
        user.accountId,
        user.keyringId,

  ...
  pg.connect(conString, function (err, client) {
    ...
    var keyringQuery = {
      text: "insert into base_keyring (\"" + columns.join('", "') + "\") \
        values (currval(pg_get_serial_sequence('account', 'account_id')), $1, \
        decode($2, 'hex'), decode($3, 'hex'), decode($4, 'hex'), \
        decode($5, 'hex'), decode($6, 'hex'), decode($7, 'hex'), \
        decode($8, 'hex'), decode($9, 'hex'), decode($10, 'hex'), decode($11, 'hex')) \
    ...
    client.query(keyringQuery, console.log);
